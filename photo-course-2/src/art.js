...
const IC = {
  ...
  play:'M12 2a10 10 0 1 0 0 20 10 10 0 0 0 0-20Zm-2 5.2 7 4.8-7 4.8V7.2Z',
  film:'M3 3h18a1 1 0 0 1 1 1v16a1 1 0 0 1-1 1H3a1 1 0 0 1-1-1V4a1 1 0 0 1 1-1Zm2 3v2h2V6H5Zm12 0v2h2V6h-2ZM5 11v2h2v-2H5Zm12 0v2h2v-2h-2ZM5 16v2h2v-2H5Zm12 0v2h2v-2h-2ZM9 6v12h6V6H9Z',
  cut:'M9.6 12 4 5.6l1.5-1.3 5.6 6.4 2-2.3 1.5 1.3-2 2.3 2 2.3-1.5 1.3-2-2.3-5.6 6.4L4 18.4 9.6 12Zm8.9 4a3 3 0 1 1 0 6 3 3 0 0 1 0-6Zm0 2a1 1 0 1 0 0 2 1 1 0 0 0 0-2Zm0-16a3 3 0 1 1 0 6 3 3 0 0 1 0-6Zm0 2a1 1 0 1 0 0 2 1 1 0 0 0 0-2Z',
  music:'M20 3v12.5a3.5 3.5 0 1 1-2-3.16V7l-8 1.6v9.9a3.5 3.5 0 1 1-2-3.16V6l12-3Z',
  list:'M4 5h2v2H4V5Zm4 0h12v2H8V5ZM4 11h2v2H4v-2Zm4 0h12v2H8v-2ZM4 17h2v2H4v-2Zm4 0h12v2H8v-2Z',
  move:'M12 2 8.5 5.5 11 5.5v5.5H5.5V8.5L2 12l3.5 3.5V13H11v5.5H8.5L12 22l3.5-3.5H13V13h5.5v2.5L22 12l-3.5-3.5V11H13V5.5h2.5L12 2Z',
  people:'M9 11a4 4 0 1 0 0-8 4 4 0 0 0 0 8Zm8 0a3 3 0 1 0 0-6 3 3 0 0 0 0 6ZM2 20c0-3.3 3.1-6 7-6s7 2.7 7 6v1H2v-1Zm16.3-4.8c2.2.8 3.7 2.5 3.7 4.8v1h-4v-1c0-1.7-.6-3.2-1.6-4.4l1.9-.4Z'
};
...
/* ── 景別：同一個商品，四種距離 ─────────────────────────────────── */
const SHOT = {
  wide:   { px:0.30, pos:'center', clutter:1, label:'全景',  en:'看整體' },
  medium: { px:0.52, pos:'center', clutter:0, label:'中景',  en:'商品＋手' },
  close:  { px:0.78, pos:'center', clutter:0, label:'近景',  en:'商品是主角' },
  detail: { px:1.35, pos:'center', clutter:0, label:'特寫',  en:'只看一個細節' }
};
ART.shot = (kind, w = 250, h = 320, o = {}) => {
  const s = SHOT[kind] || SHOT.medium;
  return ART.scene(Object.assign({
    w, h, px: Math.round(Math.min(w, h) * s.px),
    bg: o.bg || 'wood', light: o.light || 'side', angle: o.angle || 'a45',
    pos: s.pos, clutter: o.clutter != null ? o.clutter : s.clutter,
    cap: o.cap === null ? undefined : (o.cap || `${s.label}　${s.en}`),
    cls: o.cls, mark: o.mark
  }));
};
ART.SHOTS = SHOT;

/* ── 運鏡：靠近／離開／平移 ─────────────────────────────────────── */
ART.move = (kind, w = 300, h = 200) => {
  const arrow = {
    in:  `<svg style="position:absolute;inset:0" viewBox="0 0 100 100" preserveAspectRatio="none">
            <path d="M14 50h58" stroke="${C.gold}" stroke-width="3.4" fill="none"/>
            <path d="M64 42l12 8-12 8z" fill="${C.gold}"/>
            <rect x="8" y="20" width="84" height="60" fill="none" stroke="#fff"
                  stroke-width="1.6" stroke-dasharray="4 3" opacity=".7"/>
            <rect x="30" y="34" width="40" height="32" fill="none" stroke="${C.gold}"
                  stroke-width="2.2"/></svg>`,
    out: `<svg style="position:absolute;inset:0" viewBox="0 0 100 100" preserveAspectRatio="none">
            <path d="M86 50H28" stroke="${C.gold}" stroke-width="3.4" fill="none"/>
            <path d="M36 42l-12 8 12 8z" fill="${C.gold}"/>
            <rect x="30" y="34" width="40" height="32" fill="none" stroke="#fff"
                  stroke-width="1.6" stroke-dasharray="4 3" opacity=".7"/>
            <rect x="8" y="20" width="84" height="60" fill="none" stroke="${C.gold}"
                  stroke-width="2.2"/></svg>`,
    pan: `<svg style="position:absolute;inset:0" viewBox="0 0 100 100" preserveAspectRatio="none">
            <path d="M16 50h62" stroke="${C.gold}" stroke-width="3.4" fill="none"/>
            <path d="M70 42l12 8-12 8z" fill="${C.gold}"/>
            <rect x="6" y="24" width="42" height="52" fill="none" stroke="#fff"
                  stroke-width="1.6" stroke-dasharray="4 3" opacity=".7"/>
            <rect x="52" y="24" width="42" height="52" fill="none" stroke="${C.gold}"
                  stroke-width="2.2"/></svg>`
  }[kind] || '';
  return `<div class="fr" style="width:${w}px;height:${h}px">
    <div class="fr-in" style="background:linear-gradient(#c9a271,#b98f5c)">
      <div style="position:absolute;left:50%;top:52%;transform:translate(-50%,-50%)">
        ${ART.jar45(Math.round(Math.min(w,h)*0.42))}</div>
      ${arrow}
    </div></div>`;
};
...
/* ── 分鏡表 ─────────────────────────────────────────────────────── */
ART.board = (rows, o = {}) => `
<table class="tbl${o.compact ? ' compact' : ''}" style="font-size:${o.fs || 20}px">
  <thead><tr>
    <th style="width:78px">鏡頭</th><th style="width:270px">我要拍什麼</th>
    <th style="width:150px">畫面</th><th style="width:110px">秒數</th>
  </tr></thead>
  <tbody>
    ${rows.map((r,i)=>`
    <tr><td class="lead-col" style="text-align:center">${r[0] || (i+1)}</td>
      <td>${r[1]}</td><td><b>${r[2]}</b></td><td>${r[3]}</td></tr>`).join('')}
  </tbody></table>`;

/* 空白分鏡表（附錄用） */
ART.boardBlank = (n = 8) => ART.board(
  Array.from({length:n}, (_,i)=>[i+1,'&nbsp;','&nbsp;','&nbsp;']), { compact:true, fs:19 });

/* ── InShot 介面示意 ────────────────────────────────────────────── */
ART.inshot = (o = {}) => {
  const clips = o.clips || [3,2,4,2,3];
  const total = clips.reduce((a,b)=>a+b,0);
  /* 預覽區高度＝手機高 − 頂列 44 − 底部（時間軸＋工具列）150 */
  const ph = Math.max(120, Math.round((o.h || 420) - 44 - 150 - 14));
  const pw = Math.round(ph * 9 / 16);
  return `
  <div style="position:relative;width:100%;height:100%;background:#0f0f12">
    <div style="position:absolute;left:0;right:0;top:0;height:44px;display:flex;
      ...
      <span style="font-size:16px;font-weight:700;color:rgba(255,255,255,.55)">${o.left || '⟨'}</span>
      <span style="font-size:16px;font-weight:800;color:${C.gold}">${o.right || '儲存'}</span>
    </div>
    <div style="position:absolute;left:0;right:0;top:44px;bottom:150px;
         display:grid;place-items:center;background:#17171b">
      <div style="position:relative;width:${pw}px;height:${ph}px;
           border-radius:8px;overflow:hidden">
        ${ART.scene({ w:pw, h:ph, bg:'wood', light:'side', angle:'a45', px:Math.round(pw*0.5) })}
        ${o.caption ? `<div style="position:absolute;left:0;right:0;bottom:14px;text-align:center;
             font-size:13px;font-weight:900;color:#fff;text-shadow:0 1px 4px rgba(0,0,0,.8)">
             ${o.caption}</div>` : ''}
      </div>
    </div>
    <div style="position:absolute;left:0;right:0;bottom:62px;height:52px;padding:0 10px;
         display:flex;align-items:center;gap:3px">
      ${clips.map((c,i)=>`
        <div style="flex:${c};height:40px;border-radius:5px;overflow:hidden;position:relative;
             border:${o.sel===i?`2px solid ${C.gold}`:'1px solid rgba(255,255,255,.18)'}">
          <div style="position:absolute;inset:0;background:linear-gradient(${
            ['#b98f5c','#8f7f6b','#c9a271','#7d6d5b','#a98f6c'][i%5]},#6d5c4a)"></div>
          <span style="position:absolute;right:3px;bottom:2px;font-size:10px;font-weight:800;
                color:rgba(255,255,255,.75)">${c}s</span>
        </div>`).join('')}
    </div>
    <div style="position:absolute;left:10px;bottom:118px;font-size:12px;font-weight:800;
         color:rgba(255,255,255,.5)">共 ${total} 秒</div>
    <div class="edit-bar" style="position:absolute;left:0;right:0;bottom:0;border-radius:0;
         gap:3px;padding:8px 6px">
      ${['剪輯','文字','貼圖','音樂','濾鏡','畫布'].map((t,i)=>`
        <span class="eb-i${o.tab===i?' on':''}"
              style="flex:1;width:auto;min-width:0;padding:0;font-size:12px;
                     letter-spacing:0;white-space:nowrap">${t}</span>`).join('')}
    </div>
  </div>`;
};
...
/* ── 直式 9:16 影格 ─────────────────────────────────────────────── */
ART.vshot = (o = {}) => {
  const w = o.w || 176, h = Math.round(w * 16 / 9);
  return ART.scene(Object.assign({ w, h, px: Math.round(w * (o.k || 0.62)) }, o.sim || {},
    { cap: o.cap, cls: o.cls, mark: o.mark }));
};

/* ── 系列卡（流量用） ───────────────────────────────────────────── */
ART.seriesCard = (name, items, c) => `
<div class="card" style="padding:18px 20px;border-top:5px solid ${c}">
  <p class="k" style="font-size:23px;color:${c};margin-bottom:9px">${name}</p>
  ${items.map(t=>`<div class="yes" style="margin-bottom:6px;font-size:20px">
    <i style="color:${c}">●</i><span>${t}</span></div>`).join('')}
</div>`;
