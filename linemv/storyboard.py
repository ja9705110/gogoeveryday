...
SCENES = [
    ...
    ("S20", "五次迎新", 113.32, 116.34, [30],
     ...
     "但站的位置永遠是最前面那兩個。五年就在這五個畫面裡過去。",
     "兩人的線稿每 0.55 秒換一版（共五版）；每一版的集合手勢略有不同。",
     "帳篷與旗幟在每一版之間重新被畫一次，筆觸略有差異；旗面顏色逐年淡一點。",
...
PHOTO_SLOTS = {
    "station_names": ("S04 火車站牌", "牌面文字待定：一起北上念書那趟車的起訖站。", 3),
    "album_photos": ("S07 相簿內頁", "迎新的合照最合適；停留較久的那張放最有代表性的。", 4),
    "daily_moments": ("S13 日常小格", "8 格中可換 2–3 格為真實生活照（套房、出遊、兔子）。", 3),
    "wish_wall_photos": ("S17 願望牆", "真實的火車票、演唱會票根、地圖掃描。", 5),
    "camp_years": ("S20 五次迎新", "五年各放一張當年的工作人員合照，"
                   "是全片時間感最強的位置。", 5),
